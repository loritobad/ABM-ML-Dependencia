...
def count_vulnerables(model) -> int:
    ...
    return sum(
        1 for agent in _iter_agents(model) if getattr(agent, "is_vulnerable", False)
    )
...
def count_status(model, status: str) -> int:
    ...
    return sum(
        1
        for agent in _iter_agents(model)
        if getattr(agent, "saad_status", None) == status
    )
...
def count_grade(model, grade: str) -> int:
    ...
    return sum(
        1
        for agent in _iter_agents(model)
        if getattr(agent, "dependency_grade", None) == grade
    )
...
def count_benefit_type(model, benefit_type: str) -> int:
    ...
    return sum(
        1
        for agent in _iter_agents(model)
        if getattr(agent, "benefit_type", None) == benefit_type
    )
